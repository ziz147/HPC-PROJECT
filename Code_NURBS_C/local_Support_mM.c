...
void ls_2d_numba(Matrix IND_mask_tot, Matrix IND_mask, Matrix u1, Matrix u2, Vector U1, Vector U2, int p1_temp, int p2_temp, int n1_temp, int n2_temp, ListOfVectors *local_Support, COOMatrix *BF_Support, Vector *IND_mask_active) {
    ...
    for (int k = 0; k < IND_mask_tot.rows; k++) {
        ...
        int max_size_per_thread = u1.rows / omp_get_max_threads();

        #pragma omp parallel
        {
            int local_count = 0;
            int *local_ind_temp = malloc(max_size_per_thread * sizeof(int));
            ...
            #pragma omp critical
            {
                for (int i = 0; i < local_count; i++) {
                    ind_temp[count++] = local_ind_temp[i];
                }
            }
...
///////////// crée une nouvelle list et alloue ind avec count
            ...
            u.cols=1;
            u.depth=1;
            u.data=(double *)malloc(count * sizeof(double));
            v.cols=1;
            v.depth=1;
            v.rows=count;
            v.data=(double *)malloc(count * sizeof(double));
            ...
            matrice(P_rho_aux, (int)matrice(IND_mask_tot, k, 0), (int)matrice(IND_mask_tot, k, 1)) = 0.0;
 ...
 void ls_3d(Matrix IND_mask_tot, Matrix IND_mask, Matrix u1, Matrix u2,Matrix u3 , Vector U1, Vector U2, Vector U3, int p1_temp, int p2_temp, int p3_temp, int n1_temp, int n2_temp, int n3_temp,ListOfVectors *local_Support, COOMatrix *BF_Support, Vector *IND_mask_active){
   ...
   // mzero(BF_Support);
    ...
    #pragma omp parallel
    {
        ...
        w.depth=1;
        w.rows=count;
        w.data=(double *)malloc(count * sizeof(double));
        v.rows=count;
        v.cols=1;
        v.depth=1;
        v.data=(double *)malloc(count * sizeof(double));
        ...
        // remettre la valeur de P_rho a 0
        matrice3d(P_rho_aux, (int)matrice(IND_mask_tot, k, 0), (int)matrice(IND_mask_tot, k, 1),(int)matrice(IND_mask_tot, k, 2)) = 0.0;
        // remplir la matrice BF_support
